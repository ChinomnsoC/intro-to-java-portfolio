...
public class Moving {
    ...
    /**
     *
     * The Zoo is currently moving to a bigger location. Because of this, we need to
     * take our file with information
     * about each animal we have and sort them into the appropriate crate to save
     * space when moving.
     * The boxes are separated into Felines, Reptiles, and Birds, as listed in the
     * code in your Moving.java file.
     *
    ...
    public void movingBoxes(String filename) {
        try {
            ...
            while (animals.hasNextLine()) {
                ...
                String animalType = arrLine[0];
                String animalName = arrLine[1];

                switch (animalType) {
                    case "parrot":
                        BirdsBox.add(new Parrot(animalName));
                        break;
                    case "toucan":
                        BirdsBox.add(new Toucan(animalName));
                        break;
                    case "lion":
                        FelinesBox.add(new Lion(animalName));
                        break;
                    case "tiger":
                        FelinesBox.add(new Tiger(animalName));
                        break;
                    case "gecko":
                        ReptilesBox.add(new Gecko(animalName));
                        break;
                    case "lizard":
                        ReptilesBox.add(new Lizard(animalName));
                        break;
                    default:
                        break;
                }
    ...
    /**
     * Now that we have succesfully moved each crate of animals to the new zoo, it's
     * time to do a headcount.
     * To do this, you will want to go through each of your lists one at a time and
     * keep count of how many of
     * each type of animal you have found.
     *
     * For example, when going through the BirdsBox, we should check if
     * each object is an instanceOf a toucan or a parrot, then add to the
     * appropriate counter.
     * Then, no matter which it was, we add to the birds counter.
     *
     * At the end, you will add this information to a string with a new line after
     * every single entry.
     *
