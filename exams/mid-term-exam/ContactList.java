...
public class ContactList {
    ...
    public void addContact(String name, String email, long phone){
        String newLine = name + "," + email + "," + phone;

        Contact newLineC = parseLine(newLine);
        contacts.add(newLineC);
    ...
    public boolean removeContact(String name){
        Contact contact_to_remove = searchByName(name);

        if (contact_to_remove != null) {
    ...
    public ArrayList<Contact> searchByPhone(long phone){
        ...
        for (int i = 0; i < contacts.size(); i ++) {
            Contact theContact = contacts.get(i);

            // String theContactString = theContact.toString();
            if (theContact != null && theContact.getPhone() == phone) {
                results.add(theContact);
                return results;
            }
            return results;
        }
